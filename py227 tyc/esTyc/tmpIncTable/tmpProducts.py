'''
生成所有时间窗口内更新的产品id的临时表
'''

if '.' in __name__:
    from .tmpTemplet import TmpTemplet, getSqlInsert
else:
    from tmpTemplet import TmpTemplet, getSqlInsert

tableTmpProducts = tableTmp = 'temp_db.tmp_products'


### 0: 不含删除， 1: 含删除
sInsertBasic = [
    getSqlInsert(tableTmp, 'product', 'golaxy_product_id', containDel=0),
    getSqlInsert(tableTmp, 'product', 'golaxy_product_id'),
]

sInsert = []

class TmpProducts (TmpTemplet):
    '''
    生成所有时间窗口内更新的产品id的临时表
    '''
    def __init__ (this, db, mode=0):
        'mode : 0-不含删除， 1-含删除'
        TmpTemplet.__init__(this)
        this.db = db
        this.tableTmp = tableTmp
        mode = 1 if mode else 0
        this.sInsert = [sInsertBasic[mode], *sInsert]
