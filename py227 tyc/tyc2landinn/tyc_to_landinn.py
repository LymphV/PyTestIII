...
# path #
__path__  = os.path.dirname(os.path.abspath(""))
...
def update_landinn_team_member(team,team_dict):
    try:
        ...
        for key in team_dict:
            ...
            if not tyc_key_value == 'NULL':
                set_clause += '{} = {} ,'.format(key,tyc_key_value)
        set_clause += 'author_id = {} where author_id = {} '.format(strFormat(team['author_id']).mysql_format(),strFormat(team['author_id']).mysql_format())
    ...
    try: 
        ...
        for i,staff in staff_list.iterrows():
            ...
            if staff['type'] == 1 :
                ...
                # 作者 # 
                # 规格化tyc职位 #
                position = ast.literal_eval(staff['typeJoin'])
                position = ';'.join(strFormat(i,'allSpace').mysql_format() for i in position)
...
#     patent_applicationNameList = patent_applicationNameList.split(",") # landinn申请人 #
...
def get_landinn_patent_id(patent):
    ...
    try:
        "判断landinn是否存在该专利"
        landinn_patent = query_data(conn,"landinn.patent","where is_deleted = 0 and publication_number = {};".format(strFormat(patent['pubNumber']).mysql_format()))
        "不存在则在landinn新增"
