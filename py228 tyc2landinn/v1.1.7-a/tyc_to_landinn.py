...
def insert_data(conn,table,value_clause):
    try:
        ...
        traceback.print_exc(file=open(errorLogName, 'a+',encoding='UTF-8')) 
...
def merge_patent(eid,aff_id):
    try:
        ...
        for i,patent in patent_list.iterrows():      
            ...
            patent_authors_list = query_data(conn,'landinn.patent_authors','where is_deleted !=1 and patent_id = '+ str(patent_id))
            # 比较天眼和现存landinn发明人数量，不足则补全 #
            patent_inventor = patent['inventor']
            if patent_inventor and not patent_inventor.isspace():
                patent_inventor_list = set(patent_inventor.replace(",",";").split(";"))
            ...
            patent_applicants_list = query_data(conn,'landinn.patent_applicants','where is_deleted !=1 and patent_id = '+ str(patent_id))
            ...
            print(i+1, end=' ',file=open(errorLogName, 'a+',encoding='UTF-8'))
        print('',file=open(errorLogName, 'a+',encoding='UTF-8'))
    except UserMysqlException as e:
...
def start_execute():
    ...
    print('*************************')
    print('*************************','\n',file=open(errorLogName, 'a+',encoding='UTF-8'))
    print('本次需融合',execute_times,'批数据','\n',file=open(errorLogName, 'a+',encoding='UTF-8'))
    print('*************************',file=open(errorLogName, 'a+',encoding='UTF-8'))
    try:
        for index in range(execute_times):
            ...
            print('融合第',index+1,'批企业',file=open(errorLogName, 'a+',encoding='UTF-8'))
...
def tyc_to_landinn(lu_time,tu_time,record_df,start_num):
    ...
    print('查询到待融合企业数量：', len(e_list),file=open(errorLogName, 'a+',encoding='UTF-8'))
        ...
        for i,einfo in e_list.iterrows():  
            with tqdm(process, leave=True, ncols=110, bar_format='{l_bar}%s{bar}%s{r_bar}' % (Fore.GREEN, Fore.RESET)) as t:           
                ...
                if einfo['name'] is  None or einfo['name'] == "":
                    print('企业',einfo['id'],'该企业数据为空','\n',file=open(errorLogName, 'a+',encoding='UTF-8'))
                    t.set_description("融合第%i个企业--%s %s "%(i+1,einfo['id'],"该企业数据为空"))
                else:
                    print('融合第',i+1,'个企业：', einfo['name'],file=open(errorLogName, 'a+',encoding='UTF-8'))
                    eid = str(einfo['id'])
                    record_df['eid'] = [eid]
                    ...
                    for description in t:
                        ...
                        if description == '1. 查询或补全企业':
                            print('\t','1. 查询或补全企业',file=open(errorLogName, 'a+',encoding='UTF-8'))
                            aff_id = get_landinn_aff_id(einfo)
                            print('\t\t','1.1. 融合机构历史名称',file=open(errorLogName, 'a+',encoding='UTF-8'))
                            insert_history_name(aff_id,einfo)
                        if description == '2. 融合核心团队':
                            print('\t','2. 融合核心团队',file=open(errorLogName, 'a+',encoding='UTF-8'))
                            merge_team_member(eid, aff_id)
                        if description == '3. 融合主要成员':
                            print('\t','3. 融合主要成员',file=open(errorLogName, 'a+',encoding='UTF-8'))
                            merge_staff(eid, aff_id)
                        if description == '4. 融合业务':
                            print('\t','4. 融合业务',file=open(errorLogName, 'a+',encoding='UTF-8'))
                            merge_product(eid, aff_id)
                        if description == '5. 融合备案域名':
                            print('\t','5. 融合备案域名',file=open(errorLogName, 'a+',encoding='UTF-8'))
                            merge_website(eid, aff_id)
                        if description == '6. 融合商标':
                            print('\t','6. 融合商标',file=open(errorLogName, 'a+',encoding='UTF-8'))
                            merge_trademark(eid, aff_id)
                        if description == '7. 融合软著':
                            print('\t','7. 融合软著',file=open(errorLogName, 'a+',encoding='UTF-8'))
                            merge_softright(eid, aff_id)
                        if description == '8. 融合专利':
                            print('\t','8. 融合专利',file=open(errorLogName, 'a+',encoding='UTF-8'))
...
def main():
    ...
    _retries_count = 0  
    _sleep_time = random.randint(10,120) 
    while True and _retries_count <= _max_retries_count:
        try:
            num = start_execute()
            if num == 1:
                break
            ...
            _retries_count += 1
            print("错误原因", e ,file=open(errorLogName, 'a+',encoding='UTF-8'))
            print("等待",_sleep_time,"秒重启")
