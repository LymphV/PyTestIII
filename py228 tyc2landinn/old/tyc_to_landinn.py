...
#天眼查数据融合
#1. 新增--20101 修改--20102 2158个企业 - 179 =  1979 - 940 = 1039 - 1 = 1038 - 879
...
# 日志文件名称 #
def errorLogName():
    return os.path.join(__path__, f'error{getToday()}.log.txt')
...
def get_landinn_patent_id(patent):
    ...
    if landinn_patent.empty: 
        ...
        "新增专利摘要"
        if patent['abstract'] is not None and patent['abstract'] is not "":
            "判断landinn是否存在该专利摘要"
            landinn_patent_abstracts = query_data(conn,"landinn.patent_abstracts","where patent_id = '" + patent_id + "';")
...
def merge_website(eid, aff_id):
    ...
    for i,website in website_list.iterrows():
        ...
        "website_homepage"        
        "天眼查该字段非空时新增or补充"
        if website['website'] is not None or website['website'] == "":  
...
def merge_softright(eid, aff_id):
    ...
    for i,softright in softright_list.iterrows():
        ...
        softright_id = get_landinn_softright(softright,softright_dict)

        
"新增softright,并返回softright_id"
def get_landinn_softright(softright,softright_dict):   
    "查询landinn中是否存在该软著"
...
def merge_patent(eid,aff_id):
    ...
    for i,patent in patent_list.iterrows():        
        ...
        if len(patent_authors_list) < len(patent['inventor'].split(';')) :
            inventor_list = patent_authors_list.original_author
            seq = 0
            for inventor in patent['inventor'].split(';'):
        ...
        app_name_list = ast.literal_eval(patent['applicationNameList'])
...
def start_execute():
    ...
    for index in range(execute_times):
        ...
        tyc_to_landinn(lu_time,tu_time,record_df,start_num)
...
def tyc_to_landinn(lu_time,tu_time,record_df,start_num):
    ...
    try :
        ...
        for i,einfo in e_list.iterrows():
            ...
            conn.commit()     
            record_df['this_updated_time'] = tu_time
            record_df['last_updated_time'] = str(einfo['updated_at'])
            record_df['execute_complete_time'] = datetime.datetime.now(timezone(timedelta(hours=8))).strftime("%Y-%m-%d %H:%M:%S")
            record_df['eid'] = eid
            
            target_file = open(expert_file, 'w+', encoding='utf-8')
            out = record_df.to_json(orient='records')[1:-1].replace('},{', '} {')
