...
def start_execute():
    ...
    for index in range(execute_times):
        ...
        exception = tyc_to_landinn(lu_time,tu_time,record_df,start_num)
        if exception == 1:
            raise UserMysqlExceptin('执行异常','tyc_to_landinn','程序执行错误',str(exception))  
...
def tyc_to_landinn(lu_time,tu_time,record_df,start_num):
    ...
    try :
        ...
        for i,einfo in e_list.iterrows():
            ...
            last_updated_time = einfo['updated_at']
            
            record_df['this_updated_time'] = tu_time
            record_df['last_updated_time'] = str(last_updated_time)
