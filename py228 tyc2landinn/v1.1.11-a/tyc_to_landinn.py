...
def format_conversion(element):
    ...
    if element is None or element == "" :
        ...
        element = strQ2B(element)
        element = element.replace("'","''")
        element = "'" + element + "'"        
    elif math.isnan(element)  :
        return 'NULL'  
    else :
        return strQ2B(str(element))   
    return element
    ...
    try:
        ...
        for i,patent in patent_list.iterrows(): 
            ...
            if patent_applicationNameList and not patent_applicationNameList.isspace():
                ...
                app_name_list = ';'.join(strQ2B(i) for i in list)
...
def merge_patent_application(patent_applicants_list,patent_application,patent_id):
    try:
        ...
        applicant_list = list(set(strQ2B(app) for app in applicant_list))
        seq = 0
        for applicant in patent_application:
            seq += 1
            applicant = strQ2B(applicant)
            # 企业名 #
            if len(applicant) >= 4:
                # 补充专利申请人 #
...
def update_patent_applicants(papplicant,papplicant_dict):
    ...
    try:
        ...
        for key in papplicant_dict:
            ...
            tyc_key_value = format_conversion(papplicant.get(tyc_key))
            if not tyc_key_value == 'NULL':
                set_clause += '{} = {} ,'.format(key,tyc_key_value)
        set_clause += 'patent_id = {} where patent_id = {} and applicant_id = {} '.format(format_conversion(papplicant['patent_id']),format_conversion(papplicant['patent_id']),format_conversion(papplicant['applicant_id']))
...
def get_landinn_patent_id(patent):
    ...
    try:
        "判断landinn是否存在该专利"
        landinn_patent = query_data(conn,"landinn.patent","where is_deleted = 0 and publication_number = '" + patent['pubNumber'] + "';")
        "不存在则在landinn新增"
        if landinn_patent.empty: 
            ...
            if patent['abstract'] is not None and not patent['abstract'] == "":
                "判断landinn是否存在该专利摘要"
                landinn_patent_abstracts = query_data(conn,"landinn.patent_abstracts","where is_deleted = 0 and patent_id = '" + str(patent_id) + "';")
                if landinn_patent_abstracts.empty:
                    insert_landinn_patent_abstracts(patent)
                else:
