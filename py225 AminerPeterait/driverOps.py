...
switchLabel : 切换标签页
waitTillOpen : 等待页面打开
waitTillLoaded : 等待页面加载完成
closeAllOther : 关闭标签页直到剩余一个
'''
...
def waitTillLoaded (dv, secs = 60, by = By.XPATH, value = None, waitUnit = 2):
    '''
    等待页面加载完成
    ...
    waitUnit: 至少等待时间
    '''
    pathLoading = '//*[contains(@class,"sk_chase")]' if value is None else value
    sleep(waitUnit)
    ###等待加载标记消失
    for i in range(secs * 4):
        if not dv.find_elements_by_xpath(pathLoading): break
        sleep(0.25)
    if dv.find_elements_by_xpath(pathLoading): return False
    return True
    
   
def closeAllOther (dv):
    '''
    关闭标签页直到剩余一个
    '''
    switchLabel (dv, -1)
    while 1 < len(dv.window_handles):
        dv.close()
        switchLabel (dv, -1)